...
<svelte:head>
	...
	<meta
		name="description"
		content="Add a rule set for playing Hide and Seek in your location here!"
	/>
</svelte:head>
...
<ContentBox>
	...
	<div class="info">
		...
		<p>
			You will need to reference a rulebook from the <a
				class="inline-link"
				href="https://store.nebula.tv/products/hideandseek"
				target="_blank">physical copy of Hide+Seek</a
			>
			or the
			<a
				class="inline-link"
				href="https://jetlag.collinj.dev/docs/quick_start_guide/"
				target="_blank">unofficial online version by Collin James</a
			>.
		</p>
	</div>
	<form class="input-box" onsubmit={handleSubmit}>
		...
		<hr />
		<div class="input-field">
			<label class="hint" for="place">Town, city, or district:</label><input
				class="place"
				id="place"
				type="text"
		...
		<div class="input-field">
			...
			<input
				class="players"
				aria-label="Recommended team count"
				id="teams"
				type="number"
				min="2"
				max="6"
				required
				disabled={submitting}
				bind:value={fields.teams}
			/>
			<i>teams of</i>
			<input
				class="players"
				aria-label="Recommended players per team"
				id="players"
				type="number"
				min="1"
				max="6"
